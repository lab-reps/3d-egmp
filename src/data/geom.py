...
def rd_mol_to_data(mol: Mol, smiles=None):
    assert moil.GetNumConformers()==1
    N=mol.GetNumAtoms()
    ...
    data=Data(atom_type=z, pos=pos, edge_index=edge_index, edge_type=edge_type, rdmol=deepcopy(mol), smiles=smiles)
    return data

def load_confs_from_filelist(base_path, file_list, conf_per_mol):
    bad_case=0
    res=[]
    for i,f in enumerate(file_list):
        if i%10000==0: print("processed %d files"%i)
        ...
        for conf_id in conf_ids:
            ...
            labels={
                'totalenergy': conf_meta['total_energy'],
...
def idx2list(list, idx):
    return [list[x] for x in idx]
...
def gen_train_val(base_path, datasets, conf_per_mol=10, val_num=200, workers=10, seed=None, test_mask=None):
    ...
    smiles_list=[]
    pickle_path_list=[]
    num_mols, num_confs=0,0
    for dataset_name in datasets:
        summary_path=os.path.join(base_path, 'summary_%d.json'%dataset_name)
        with open(summary_path, 'r') as f:
            summary=json.load(f)

        for smiles, meta_mol in summary.items():
            ...
            if pickle_path is None:
                continue
            if u_conf<1 or smiles in test_mask:
                continue
            num_mols+=1
    ...
    split_idxs=list(range(len(pickle_path_list)))
    rasndom.shuffle(split_idxs)
    val=split_idxs[:val_num]
    train=split_idxs[val_num:]
...
def gen_GEOM_blocks(base_path, datasets, output_dir, conf_per_mol=10, val_num=200, train_block_size=int(1e5), workers=10, seed=None, test_mask=None):
    ...
    os.makedirs(os.path.join(base_path, '..', outout_dir), exist_ok=True)
...
if __name__ == '__main__':
    parser=argparse.ArgumentParser(descriptio='GeomData')
    ...
    parser.add_argument('--block_size', type=int, help='conformations per block', default=int(1e5))
    ...
    if len(args.test_smiles)>0:
        with open(args.test_smiles, 'r') as f:
            lines=f.readlines()
        test_mask=[line.strip() for line in lines]
    else:
        test_mask=None
    gen_GEOM_blocks(args.base_path, args.datasets, args.output, val_num=args.val_num, train_block_size=args.block_size, conf_per_mol=args.conf_num, test_mask=test_mask,)